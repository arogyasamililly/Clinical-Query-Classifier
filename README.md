...
| **JIRA Integration** | Auto-creates a ticket when site confusion rate exceeds 30%. |
...
- (Optional) JIRA access for auto-ticket creation
...
The `.env` file (or `run.bat`) should contain:

| Variable | Description | Required |
|----------|-------------|----------|
| `JIRA_SERVER` | Your JIRA instance URL (e.g., `https://jira.lilly.com`) | Optional |
| `JIRA_PROJECT` | JIRA project key (e.g., `BIOBOT`) | Optional |
| `JIRA_USER` | Service account email | Optional |
| `JIRA_TOKEN` | API token for the service account | Optional |
| `JIRA_ISSUE_TYPE` | Issue type (default: `Task`) | Optional |

> JIRA integration is optional. The app works fully without it — it simply skips ticket creation.

### Running the App

**Option 1: Using run.bat (Windows)**

Edit `run.bat` with your JIRA credentials, then:
...
- If confusion rate exceeds 30%, triggers JIRA ticket creation

### JIRA Integration

When the overall confusion rate exceeds 30%, a JIRA ticket is automatically created containing:

- Total query count and classification breakdown
- Top CRF items causing confusion
- Recommendation for the team to review query wording or site training

This is optional and requires JIRA environment variables to be set.
...
|----------|---------------|----------------|
| Classification Distribution | Pie/bar chart of category counts | Quick overview of response patterns |
...
| JIRA ticket not created | Missing environment variables | Set JIRA vars in `.env` or `run.bat` |
